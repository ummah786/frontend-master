...
import { BiClipboard, BiPhoneCall, BiPhoneOff, BiDesktop, BiVideoOff, BiVideo } from "react-icons/bi";
import { SocketContext } from "../Context";

const Options = () => {
    const { me, callAccepted, name, setName, callEnded, leaveCall, callUser, shareScreen, screenSharing, toggleCamera, stream } = useContext(SocketContext);
        ...
        <Container maxW="1200px" m="35px 0" p="0">
            <Box p="10px" border="2px" borderColor="black" borderStyle="solid">
                <FormControl display="flex" flexDirection="column" noValidate aria-autocomplete="none">
                        ...
                        <Grid colSpan={1} p="6">
                            ...
                            <Input type='text' value={idToCall} onChange={(e) => setIdToCall(e.target.value)} width="100%" />
                            {
                                callAccepted && !callEnded ? (
                                    <Button leftIcon={<BiPhoneOff />} onClick={leaveCall} mt="20" colorScheme='teal' variant='solid'>
                                        Hang up
                                    </Button>
                                ) : (
                            ...
                            }
                            {
                                callAccepted && !callEnded && !screenSharing && (
                                    <Button leftIcon={<BiDesktop />} onClick={shareScreen} mt="20" colorScheme='teal' variant='solid'>
                                        Share Screen
                                    </Button>
                                )
                            }
                            {
                                callAccepted && !callEnded && (
                                    <Button leftIcon={stream && stream.getVideoTracks()[0].enabled ? <BiVideoOff /> : <BiVideo />} onClick={toggleCamera} mt="20" colorScheme='teal' variant='solid'>
                                        {stream && stream.getVideoTracks()[0].enabled ? 'Turn Off Camera' : 'Turn On Camera'}
                                    </Button>
                                )
                            }
                        </Grid>
                    ...
                    {/!* Display Name and ID *!/}
                    <Box mt={6}>
                        <Heading as="h6" size="sm">Your Name: {name}</Heading>
                        <Heading as="h6" size="sm">Your ID: {me}</Heading>
                    </Box>
                </FormControl>
            </Box>
        </Container>
